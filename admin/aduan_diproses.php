<?php 
$pengaduan = array();
$ambil = $koneksi->query("SELECT * FROM pengaduan LEFT JOIN daftar_pengaduan ON daftar_pengaduan.id_pengaduan = pengaduan.id_pengaduan LEFT JOIN masyarakat ON masyarakat.id_masyarakat = pengaduan.id_masyarakat LEFT JOIN terlapor ON terlapor.id_terlapor = pengaduan.id_terlapor WHERE status_pengaduan='Diproses'");
					...
					<tbody>
						<?php foreach ($pengaduan as $key => $value): ?>
							<tr>
								...
								<td>
									<a href="index.php?page=aduan_diproses_detail&id=<?php echo $value['id_pengaduan']; ?>" class="btn btn-info btn-sm shadow-sm"><i class="bi bi-pencil-square"></i></a>
								</td>
							</tr>
						<?php endforeach ?>
					</tbody>
